...
try {
    ...
    // Agregar logs de depuración para ver todos los datos recibidos
    file_put_contents('php://stderr', "=== DATOS COMPLETOS DE LA ORDEN ===\n");
    file_put_contents('php://stderr', json_encode($orderData, JSON_PRETTY_PRINT) . "\n");
    file_put_contents('php://stderr', "=== FIN DATOS DE LA ORDEN ===\n");
    
    // Verificar específicamente el campo idReal
    if (isset($orderData['idReal'])) {
        file_put_contents('php://stderr', "✅ idReal encontrado: " . $orderData['idReal'] . "\n");
    } else {
        file_put_contents('php://stderr', "❌ idReal NO encontrado en los datos\n");
        file_put_contents('php://stderr', "Claves disponibles: " . implode(", ", array_keys($orderData)) . "\n");
    }
    ...
    // BUCLE PARA IMPRIMIR 2 COPIAS DEL TICKET
    for ($copia = 1; $copia <= 2; $copia++) {
        file_put_contents('php://stderr', "Imprimiendo copia $copia de 2\n");
        
        // Configuración inicial
        $printer->setJustification(Printer::JUSTIFY_CENTER);

        // Logo (opcional) - SE IMPRIME EN CADA COPIA
        try {
            file_put_contents('php://stderr', "==== DEPURACIÓN AVANZADA LOGO (COPIA $copia) ====\n");
                                ...
                                if (!$result) {
                                    ...
                                    if (!$saveResult) {
                                        ...
                                        try {
                                            ...
                                            unlink($tempPath);
                                            file_put_contents('php://stderr', "✅ Logo enviado a la impresora (copia $copia)\n");
                                        } catch (Exception $e) {
            ...
            file_put_contents('php://stderr', "==== FIN DEPURACIÓN AVANZADA LOGO (COPIA $copia) ====\n");
        } catch (Exception $e) {
        ...
        // Agregar número de orden (idReal)
        if (isset($orderData['idReal'])) {
            $printer->setEmphasis(true);
            $printer->text("ORDEN N: " . $orderData['idReal'] . "\n");
        ...
        // Agregar separación entre copias (excepto en la última)
        if ($copia < 2) {
            $printer->feed(2);
            file_put_contents('php://stderr', "Copia $copia completada\n");
        }
    }

    $printer->pulse();
    $printer->close();
    file_put_contents('php://stderr', "Impresión de 2 copias completada exitosamente\n");
