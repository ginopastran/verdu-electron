...
ipcMain.handle("print-ticket", async (_, orderData) => {
    try {
        ...
        try {
            ...
            const printerFound = availablePrinters.some((printer) => printer.name === targetPrinter && printer.status === 0 // 0 = idle/ready
            );
            if (!printerFound) {
                console.log(`❌ Impresora '${targetPrinter}' no encontrada o no disponible`);
                return {
                    success: false,
...
ipcMain.handle("print-closing", async (_, closingData) => {
    try {
        ...
        try {
            ...
            if (!printerFound) {
                console.log(`❌ Impresora '${targetPrinter}' no encontrada o no disponible para cierre`);
                return {
                    success: false,
                    printerError: `Impresora '${targetPrinter}' no está conectada o no está disponible`,
                    message: "Error de impresión: Impresora no disponible",
            ...
            console.log(`✅ Impresora '${targetPrinter}' encontrada y disponible para cierre`);
        }
        catch (printerCheckError) {
            console.error("❌ Error al verificar impresoras para cierre:", printerCheckError);
            return {
                success: false,
                printerError: "No se pudo verificar el estado de las impresoras",
                message: "Error de impresión: Sistema de impresión no disponible",
        ...
        const mostrarEstructura = (obj, prefijo = "") => {
            for (const key in obj) {
                if (obj.hasOwnProperty(key)) {
                    const valor = obj[key];
                    const tipo = Array.isArray(valor) ? "array" : typeof valor;
                    if (tipo === "object" && valor !== null) {
                        console.log(`${prefijo}${key} (${tipo}${Array.isArray(valor) ? `[${valor.length}]` : ""})`);
        ...
        console.log(`Fecha inicio: ${new Date(datosAdaptados.fechaInicio).toLocaleString()}`);
        console.log(`Fecha cierre: ${new Date(datosAdaptados.fechaCierre).toLocaleString()}`);
        console.log("-------------------------------------");
        // MOSTRAR VENTAS POR MÉTODO DE PAGO
        console.log("VENTAS POR MÉTODO DE PAGO:");
        if (datosAdaptados.ventasPorMetodo &&
            typeof datosAdaptados.ventasPorMetodo === "object") {
            Object.entries(datosAdaptados.ventasPorMetodo).forEach(([metodo, total]) => {
        ...
        if (datosAdaptados.ventasPorVendedor &&
            Array.isArray(datosAdaptados.ventasPorVendedor)) {
            datosAdaptados.ventasPorVendedor.forEach((vendedor) => {
                console.log(`${vendedor.nombre}: $${Number(vendedor.totalVentas).toLocaleString()} (${vendedor.cantidadVentas} ventas)`);
        ...
        console.log("-------------------------------------");
        console.log(`TOTAL: $${Number(datosAdaptados.totalVentas).toLocaleString()} (${datosAdaptados.cantidadVentas} ventas)`);
        console.log("======================================");
        // Escribir datos para la impresión
        await fsPromises.writeFile(tempDataPath, JSON.stringify(datosAdaptados), "utf8");
        // Rutas del PHP y logo
        const isProduction = process.env.NODE_ENV !== "development";
        let phpScriptPath;
        if (isProduction) {
