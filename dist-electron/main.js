...
ipcMain.handle("print-factura-ticket", async (_, facturaData) => {
    try {
        ...
        await fsPromises.writeFile(tempDataPath, JSON.stringify(facturaData), "utf8");
        const isProduction = process.env.NODE_ENV !== "development";
        let phpScriptPath;
        if (isProduction) {
            phpScriptPath = path.join(process.resourcesPath, "resources", "factura_ticket_printer.php");
        }
        else {
            phpScriptPath = path.join(app.getAppPath(), "resources", "factura_ticket_printer.php");
        }
        // LOG: Verificar existencia del script PHP
        if (!fs.existsSync(phpScriptPath)) {
            console.error("❌ No se encontró el script PHP de factura:", phpScriptPath);
            return {
                success: false,
